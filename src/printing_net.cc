...
//	for(int i=0;i<S.NG;i++) stream<<*S.g[i];//	cerr<<"Printing grain: "<<*S.g[i]<<endl;}
    ...
    for(int i=0;i<S.NP;i++) S.p[i]->tmp=S.p[i]->perm(&S);
    for(int i=0;i<S.NP;i++) stream<<*S.p[i];// 	cerr<<"Printing pore: "<<*S.p[i]<<endl;}
    for(int i=0;i<S.NN;i++) S.n[i]->tmp=S.n[i]->x; //S.distance_to_root(S.n[i]);//S.n[i]->x;   //FIXME: This will get us in the inf loop
...
void Print_network_in_dissolution_style (ofstream_ps & stream, Network &S){
    ...
    stream<<"0 0 moveto "<<S.N_x<<" 0 lineto "<<S.N_x<<" "<<-S.N_y<<" lineto "<<0<<" "<<-S.N_y<<" lineto "<<0<<" 0 lineto closepath"<<endl;
